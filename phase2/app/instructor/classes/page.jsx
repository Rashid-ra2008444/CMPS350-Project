...
export default function InstructorClasses() {
  ...
  useEffect(() => {
    ...
    loadInstructorClasses(session.user.username);
  ...
  const handleViewGrades = (course) => {
    ...
    router.push(
      `/instructor/grading?courseNum=${course.courseNum}&crn=${course.crn}`
    );
  };
  ...
  return (
    <>
      <section className="banner">
        <h1 className="title">
          Welcome <span id="instructor-name">{session?.user?.username}</span>
        </h1>
        <h2>My Classes</h2>
      </section>
              ...
              return (
                ...
                >
                  ...
                  {canGrade ? (
                    ...
                    <p className={styles.gradingNotice}>
                      ⚠️ Grading unavailable until course is approved
                    </p>
                  )}
                </div>
              );
