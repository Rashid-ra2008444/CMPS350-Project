...
// ✅ NEW: StudentRepository to handle Student-specific operations
...
class CourseRepository {
  ...
  async findByCRN(crn) {
    return await prisma.course.findUnique({
      ...
      include: {
        enrollments: {
          // ✅ NEW: Include student relation in enrollments
          include: {
            student: {
              include: {
                user: true,
...
class EnrollmentRepository {
  ...
  async findAll() {
    return await prisma.enrollment.findMany({
      include: {
        course: true,
        // ✅ NEW: Include student with user relation
        student: {
          include: {
            user: true,
          },
  ...
  async create(enrollmentData) {
    // Debug log
    console.log(
      "Creating enrollment with studentId:",
      enrollmentData.studentId
    );

    // تحقق أنه فعلاً رقم
    if (!enrollmentData.studentId || isNaN(Number(enrollmentData.studentId))) {
      throw new Error("❌ studentId is missing or invalid");
    }
    ...
    return await prisma.enrollment.create({
      data: {
        ...
        course: {
          connect: { crn: Number(enrollmentData.crn) },
        },

        student: {
          connect: { studentId: Number(enrollmentData.studentId) },
        },
      },
    });
  }
...
export const courseRepository = new CourseRepository();
export const enrollmentRepository = new EnrollmentRepository();
// ✅ NEW: Export the new StudentRepository
